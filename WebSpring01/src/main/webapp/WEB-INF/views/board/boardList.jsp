...
<style>
	...
	.wordCut{
		white-space: nowrap; /*긴 제목 안넘어가게 표시하는거*/
		overflow: hidden;
	...
	#board>li{float:left; width:10%;border-bottom:1px solid gray; height: 40px; }
...
<h1>게시판 리스트</h1>
<div id="header">
	<div>총레코드수 : ${pVo.totalRecord}</div><!-- pvo 모델에 담아서줌 -->
...
<ul id="board">
	...
	<li>등록일</li>
	<li>조회수</li>
<c:forEach var="vo" items="${list}">	<!-- 변수명에 리스트 담아서 -->
	<li>${vo.no}</li>
...
<div>
 ...
 <ul class="pagination pagination-lg">
         	...
         	<c:if test="${pVo.nowPage==1}">
        		...
        		<!-- 시작페이지부터 5개의 페이지를 출력 -->
       		  	<!-- 6은 스타트 페이지가 가지고 있음,7,8,9,10 -->
         <c:forEach var="i" begin="${pVo.startPage}" end="${pVo.startPage+pVo.onePageNumCount-1}"> <!-- end ~까지 찍히는거다. -->
         	<!-- 출력할 페이지번호가 총페이지수보다 작을때 만 출력한다. -->
         	<c:if test="${i<=pVo.totalPage}"> <!-- totalpage까지만 찍어줘라. -->
   		...
   		<!-- 지금현재페이지가 총 페이지수보다 작으면 다음페이지가 있다. -->
   		<c:if test="${pVo.nowPage<pVo.totalPage}">
   	
        	 <li class='page-item'><a href="/myapp/boardList?nowPage=${pVo.nowPage+1}<c:if test='${pVo.searchWord!=null && pVo.searchKey!=null}'>&searchKey=${pVo.searchKey}&searchWord=${pVo.searchWord}</c:if>" class='page-link'>NEXT</a></li> <!-- 지금보고있는 페이지수가 총 페이지수보다 작으면 다음페이지가 있다. -->
        </c:if>
      </ul>
